...
import {
    ensureImageUri,
    getProvider, runViewFunction, TransactionContext,
} from "../Helper";
import {
    Alert,
    Button,
...
import {V1AuctionListing, V1FixedListing, V2AuctionListing, V2FixedListing} from "./Wallet";
...
export function TokenDetails(props: { network: Network, token_id: string }) {
    ...
    useEffect(() => {
        ...
        setCtx(
            {
                network: props.network, account: walletState.account,
    ...
    return <EasyBorder offset={1}>
        <Layout>
            <Row align="middle">
                <Col offset={1}>
                    <h2>{token?.collection} : {token?.name}</h2>
                </Col>
            </Row>
            <Divider/>
            <Row align="middle">
                ...
                <Col offset={1} flex={"auto"}>
                    ...
                    >
                        <Descriptions.Item label="Standard">{token?.standard}</Descriptions.Item>
                        <Descriptions.Item label="TokenId">{props.token_id}</Descriptions.Item>
                    </Descriptions>
                </Col>
            </Row>
            <Row align={"middle"}>
                <Col offset={2} span={2} flex={"auto"}>
                    ...
                    >
                        ...
                        {ctx && token?.standard === V1 && listingType === FIXED_PRICE &&
                            <V1FixedListing item={token} ctx={ctx} submit={submitFixed}
                                            submitCallback={finishedCallback}/>}
                        {ctx && token?.standard === V1 && listingType === AUCTION &&
                            <V1AuctionListing item={token} ctx={ctx} submit={submitFixed}
                                              submitCallback={finishedCallback}/>}
                        {ctx && token?.standard === V2 && listingType === FIXED_PRICE &&
                            <V2FixedListing item={token} ctx={ctx} submit={submitFixed}
                                            submitCallback={finishedCallback}/>}
                        {ctx && token?.standard === V2 && listingType === AUCTION &&
                            <V2AuctionListing item={token} ctx={ctx} submit={submitFixed}
                                              submitCallback={finishedCallback}/>}
                    </Modal>
                </Col>
                <Col span={2} flex={"auto"}>
                    ...
                    >
                        ...
                        {ctx && token?.standard === V2 &&
                            <Transfer ctx={ctx} objectAddress={token?.data_id} submit={submitFixed}
                                      submitCallback={finishedCallback}/>}
                    </Modal>
                </Col>
            </Row>
